...
class FabricantePA{
	...
	{
		$sql="INSERT INTO fabricante(cod_fab,nome,
			endereco,telefone,cidade,cod_adm) VALUES(".
	...
	{
		$sql="SELECT COUNT(cod_fab) AS 'quantidade' 
		FROM fabricante";
		$consulta=$this->con->consultar($sql);
		$linha=$consulta->fetch_assoc();
	...
	public function buscar($termo,$campo)
	{
		if ($campo=="nome") {
			...
			$sql="SELECT * FROM fabricante WHERE
			cod_fab='$termo' OR nome LIKE '%$termo%'
			OR endereco LIKE '%$termo%' OR telefone 
			LIKE '%$termo%' OR cidade LIKE '%$termo%' 
			OR cod_adm='$termo'";
		}
		//var_dump($sql);
		$consulta=$this->con->consultar($sql);
	...
	{
		$sql="DELETE FROM fabricante WHERE 
		cod_fab=$cod_fab";
		$flag=true;
...
{
	$sql="SELECT cod_fab,nome FROM fabricante";
	$consulta=$this->cond->consultar(sql);
	$this->con->desconectar();
	return $consulta;
}
